...
BaiduCloudStorage.prototype.upload = function(bucketName,
    ...
    var checkExistedDef = this.checkExisted(data, bucketName, objectName);
    var me = this;
    checkExistedDef.done(function () {
        var realUploadDef = me.realUpload( data, bucketName, objectName);
    ...
    checkExistedDef.fail(function (targetUrl) {
        var bcsUrl = decodeURIComponent( targetUrl.replace(/\?.*/g, '') );
        var cdnHost = require( 'edp-config' ).get( 'bcs.host' );
        if (cdnHost) {
            bcsUrl = bcsUrl.replace(/^http:\/\/(s3\.bae|bs)\.baidu\.com/,
        ...
        edp.log.error('%s is already existed in %s', localFile, 'http://' + bcsUrl);
    });
    return def;
};

BaiduCloudStorage.prototype.checkExisted = function(data, bucketName, objectName) {
    ...
    var targetUrl = this.sign('HEAD', bucketName, objectName);
